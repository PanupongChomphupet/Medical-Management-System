...
const filteredHospitals = computed(() => {
  ...
  return hospitals.filter((hospital) =>
    [hospital.name, hospital.code, hospital.province, hospital.status]
      .join(' ')
      .toLowerCase()
      .includes(keyword),
  )
...
const statsCards = computed(() => [
  { label: 'Total Hospitals', value: String(hospitals.length) },
  {
    label: 'Active Hospitals',
    value: String(hospitals.filter((hospital) => hospital.status === 'Active').length),
  },
  {
    label: 'Total Equipment',
    value: hospitals.reduce((total, hospital) => total + hospital.equipment, 0).toLocaleString(),
  },
  {
    label: 'Open Work Orders',
    value: hospitals.reduce((total, hospital) => total + hospital.workOrders, 0).toLocaleString(),
  },
])
</script>
        ...
        <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
          ...
          <tbody class="divide-y divide-slate-100 bg-white">
            <tr v-for="hospital in filteredHospitals" :key="hospital.id" class="hover:bg-slate-50">
              <td class="px-5 py-4">
                <p class="font-semibold text-slate-950">{{ hospital.name }}</p>
                <p class="mt-1 text-xs text-slate-500">{{ hospital.code }}</p>
              </td>
              <td class="px-5 py-4 text-slate-600">{{ hospital.province }}</td>
              ...
              <td class="px-5 py-4">
                <div class="flex justify-end gap-2">
                  <RouterLink
                    :to="{ name: 'hospital-overview', params: { id: hospital.id } }"
